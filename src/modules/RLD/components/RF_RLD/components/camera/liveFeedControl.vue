<template>
  <div class="flex items-center justify-center space-x-3 rounded-xl">
    <div v-if="enabledItems.bottle" class="flex items-center justify-center w-12 h-12 bg-secondary rounded-xl">
      <img :src="bottleIcon" class="scale-75" alt="Bottle Icon" />
    </div>

    <div v-if="enabledItems.resolution" class="flex items-center justify-center w-12 h-12 bg-secondary rounded-xl">
      <img :src="resolutionIcon" class="scale-75" alt="Resolution Icon" />
    </div>

    <div
      v-if="enabledItems.groupBottle"
      class="flex flex-col items-center justify-center w-12 h-12 cursor-pointer bg-secondary rounded-xl"
    ...
    <div v-if="enabledItems.freeze" class="flex items-center justify-center w-12 h-12 bg-secondary rounded-xl">
      <img :src="freezeIcon" class="scale-75" alt="Freeze Icon" />
    </div>

    <div
      v-if="enabledItems.seconds"
      class="w-12 h-12 font-semibold text-[1.6vw] bg-secondary rounded-xl flex items-center justify-center cursor-pointer"
      @click="updateSeconds"
    >
      {{ seconds }}
    </div>

    <div v-if="enabledItems.save" class="flex items-center justify-center w-12 h-12 bg-secondary rounded-xl">
...
import { ref } from 'vue';
import bottleIcon from '../../../../../../assets/screenIcons/liveFeedIcons/RF_Residual_Detection.svg';
